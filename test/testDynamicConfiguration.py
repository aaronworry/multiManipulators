...
from environment.oneStaticManipulatorGrabAndErect import Env
...
def init(robots, things):
    ...
    for thing in things:
        if thing.type == 'cube':
            ...
            T2.add(thing)
    return robots[0], T1, T2

def allocate(robot, things):
    # robot
    ...
    minDis = np.inf
    for j in range(n):
        dis = cal_distance(robot.ee.get_position(), things[j].get_position())
        if dis <= minDis:
            minDis = dis
            b = j
    robot.thing = things[b]
    result.add(things[b])
    return result  # a set, things has been assigned


def taskAs(robot, T1, T2):
    task1 = list(T1)
    task2 = list(T2)
    set1 = set()
    set2 = set()
    if robot.type == 'type1':
        if robot.action == 'idle':
            set1 = allocate(robot, task1)
    else:
        if robot.action == 'idle':
            set2 = allocate(robot, task2)
    t1 = T1 - set1
    t2 = T2 - set2
    return t1, t2

env = Env(robot_config=[{'type1': 0}, {'type2': 1}], thing_config=[{'cube': 2}, {'cylinder': 2}])
episode = 3
