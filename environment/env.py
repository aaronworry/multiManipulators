...
class Env():
    ...
    def get_thing_position(self, thing_id):
        position, _ = self.p.getBasePositionAndOrientation(thing_id)
        return position

    def reset_thing_pose(self, thing_id, pos_x, pos_y, orientation):
        position = (pos_x, pos_y, 0.1)
        self.p.resetBasePositionAndOrientation(thing_id, position, heading_to_orientation(orientation))

    def remove_thing(self, thing_id):
        self.p.resetBasePositionAndOrientation(thing_id, (0, 0, -1000), (0, 0, 0, 1))
        self.removed_thing_ids_set.add(thing_id)

    def thing_position_in_receptacle(self, thing_position):
        # if thing in those goal return True else False
        pass

    def get_robot_group_type(self):
        return self.robot_group_types

    def start_video_logging(self, video_path):
        assert self.show_gui
        return self.p.startStateLogging(p.STATE_LOGGING_VIDEO_MP4, video_path)
    ...
    def _get_obstacles(self, wall_thickness):
        ...
        for x, y, length, width in [
            ...
            obstacles.append({'type': 'wall', 'position': (x, y), 'orientation': 0, 'x_len': length, 'y_len': width})
